...
public class MyTasksServiceImpl implements MyTasksService
{
    ...
    {
        String url = camundaModifiedPathURL.getCamundaPathUri(ENGINE_REST+GET_MY_TASK_COUNT);
    ...
    {
        WebClient webClient=webClientWrapper.createWebClient(tokenUtils.getTokenFromContext());
        String requestURL = camundaModifiedPathURL.getCamundaPathUri(ENGINE_REST+TASK+ "?"+ FIRST_RESULT + "=" + firstResult + "&" + MAX_RESULTS + "=" + maxResult);
    ...
    {
        WebClient webClient=webClientWrapper.createWebClient(tokenUtils.getTokenFromContext());
        String requestURL = camundaModifiedPathURL.getCamundaPathUri(ENGINE_REST+HISTORY+CASE_ACTIVITY_INSTANCE+CASE_INSTANCE_ID+caseInstanceId);
    ...
    public Map<String, Object> getMyTasksById(String id) throws JsonProcessingException {
        WebClient webClient=webClientWrapper.createWebClient(tokenUtils.getTokenFromContext());
        String requestURL = camundaModifiedPathURL.getCamundaPathUri(ENGINE_REST+URL_SEPERATOR+CASE_INSTANCE+URL_SEPERATOR+id);
        String response = webClientWrapper.webclientRequest(webClient,requestURL,GET,null);
