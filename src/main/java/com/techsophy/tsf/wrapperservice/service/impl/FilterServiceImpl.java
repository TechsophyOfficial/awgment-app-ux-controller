...
public class FilterServiceImpl implements FilterService
{
    ...
    {
        String url = gatewayURI + uxControllerCamundaServletContextPath.getCamundaPathUri() +GET_CREATE_FILTER;
    ...
    public List<Map<String, Object>> executeFilter(String id, ExecuteFilterDTO executeFilterDTO, String emptyString, String firstResult, String maxResult, boolean involvedUser) throws JsonProcessingException {
        ...
        {
            requestURL = gatewayURI + uxControllerCamundaServletContextPath.getCamundaPathUri() +GET_EXECUTE_FILTER_LIST + URL_SEPERATOR + id + URL_SEPERATOR + LIST + "?"+emptyString+"&"+ FIRST_RESULT + "=" + firstResult + "&" + MAX_RESULTS + "=" + maxResult;

        }
        else
        {
            requestURL = gatewayURI + uxControllerCamundaServletContextPath.getCamundaPathUri() +GET_EXECUTE_FILTER_LIST + URL_SEPERATOR + id + URL_SEPERATOR + LIST;
    ...
    {
        String url = gatewayURI + uxControllerCamundaServletContextPath.getCamundaPathUri() +ENGINE_REST+FILTER+URL_SEPERATOR+id;
        WebClient webClient=webClientWrapper.createWebClient(tokenUtils.getTokenFromContext());
    ...
    public FilterCountResponseDTO filterCount(String id,FilterCountDTO filterCountDTO) throws JsonProcessingException {
        String url = gatewayURI + uxControllerCamundaServletContextPath.getCamundaPathUri() +ENGINE_REST+FILTER+URL_SEPERATOR+id+MessageConstants.COUNT;
        WebClient webClient=webClientWrapper.createWebClient(tokenUtils.getTokenFromContext());
    ...
    {
        String url = gatewayURI + uxControllerCamundaServletContextPath.getCamundaPathUri() +ENGINE_REST+TASK+URL_SEPERATOR+id+FORM_VARIABLES;
        WebClient webClient=webClientWrapper.createWebClient(tokenUtils.getTokenFromContext());
        String response = webClientWrapper.webclientRequest(webClient,url,GET,null);
